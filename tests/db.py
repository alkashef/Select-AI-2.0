...
class _DBLogAdapter:
    """Adapts the external logger interface used in the copied code to the
    project's existing ChatLogger (no changes to logger.py required).

    Methods expected in original code:
      - info(msg)
      - debug(msg)
      - warning(msg)
      - error(msg)
      - log_operation(operation, state, extra: dict|None)
      - exception(exc, context)

    All map to ChatLogger.event with lightweight structured fields.
    Failures in logging are swallowed to keep DB flow resilient.
    """

    def __init__(self) -> None:
        self._logger = ChatLogger()

    def _safe_event(self, name: str, **fields: str) -> None:
        try:
            # Coerce all values to strings and strip whitespace
            norm = {k: str(v).strip() for k, v in fields.items() if v is not None}
            self._logger.event(name, **norm)
        except Exception:
            pass

    def info(self, msg: str) -> None:
        self._safe_event("db.info", msg=msg)

    def debug(self, msg: str) -> None:
        self._safe_event("db.debug", msg=msg)

    def warning(self, msg: str) -> None:
        self._safe_event("db.warning", msg=msg)

    def error(self, msg: str) -> None:
        self._safe_event("db.error", msg=msg)

    def log_operation(self, operation: str, state: str, extra: Dict[str, Any] | None = None) -> None:  # type: ignore[name-defined]
        fields = {"op": operation, "state": state}
        if extra:
            for k, v in extra.items():
                fields[k] = v
        self._safe_event("db.operation", **fields)
...
# Exposed adapter instance with the same name expected in original code
logger = _DBLogAdapter()
